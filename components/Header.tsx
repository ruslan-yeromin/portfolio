  ...
  return (
    <motion.li
      className={clsx("h-3/4 flex items-center justify-center shrink-0", {
        "text-gray-950 relative": isActive,
      })}
    ...
    >
      <Link
        href={hash}
        className="flex w-full items-center justify-center font-semibold px-3 py-3 hover:text-gray-950 transition"
        onClick={() => {
...
const Header: React.FC = () => {
  ...
  return (
    <header className="z-[999] relative">
      <motion.div
        className="fixed top-0 left-1/2 -translate-x-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[40rem] sm:rounded-full"
